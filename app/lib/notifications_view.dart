...
class NotificationsView extends StatefulWidget {
  const NotificationsView({super.key, this.onHome});

  /// Called when the user taps the home affordance to return to the shell grid.
...
class _NotificationsViewState extends State<NotificationsView>
  ...
  DateTime _oldDay = DateTime.now(); // selected day in the Old (archive) tab
  ...
  void _onTaskData(Object data) {
    ...
    try {
      ...
      // ignore: avoid_print
      print('[Aisthetron] onTaskData error: $e\n$st');
    }
  }
  ...
  Widget _buildOldTab() {
    final eventDays = <DateTime>{
      for (final n in _oldNotifications) dateOnly(n.createdAt.toLocal()),
    };
    final dayItems = _oldNotifications
        .where((n) => dateOnly(n.createdAt.toLocal()) == dateOnly(_oldDay))
        .toList();
    ...
    return Column(
      children: [
        ...
        DateNavBar(
          day: _oldDay,
          onChanged: (d) => setState(() => _oldDay = d),
          eventDays: eventDays,
        ),
        const Divider(height: 1),
        Expanded(
          child: dayItems.isEmpty
              ? const Center(child: Text('No notifications on this day.'))
              : ListView.separated(
